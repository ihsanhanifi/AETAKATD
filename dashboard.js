...
function renderGuestsTable(guests) {
    ...
    guests.forEach((g, index) => {
        ...
        // 🆕 Mendukung Base64 LAMA dan URL Google Drive BARU
        if (g.fotoSelfie && g.fotoSelfie !== 'Tidak ada foto' && (g.fotoSelfie.startsWith('data:image') || g.fotoSelfie.startsWith('https://'))) {
            photoCell = `<td class="no-print" style="text-align: center;"><img src="${g.fotoSelfie}" alt="Foto ${g.nama}" class="photo-thumbnail" onclick="openLightbox('${g.fotoSelfie}', '${g.nama} - ${date}')"></td>`;
        } else {
    ...
    try {
        ...
        const excelData = allGuests.map((g, index) => {
            ...
            return {
                ...
                'No. HP/WA': g.noHp || '-',
                'Foto': (g.fotoSelfie && g.fotoSelfie !== 'Tidak ada foto') ? 'Ada (Lihat Spreadsheet)' : 'Tidak Ada'
            };
        });
        
        // 2. Data Sheet 2: Ringkasan
        ...
        ws1['!cols'] = [{ wch: 5 }, { wch: 12 }, { wch: 10 }, { wch: 25 }, { wch: 20 }, { wch: 30 }, { wch: 20 }, { wch: 30 }, { wch: 15 }, { wch: 20 }];
